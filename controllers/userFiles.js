...
router.get('/getUserUploads/:id/:cat', async (req, res) => {
    try {
        ...
        UserFiles.belongsTo(Client, {foreignKey: 'client'})
        UserFiles.belongsTo(User, {foreignKey: 'user'})
        const files = await UserFiles.findAll({where: {client: id, category:cat},
        include: [Client, User]});
        res.json(files);
    }
    catch (err) {
...
router.get('/deleteUserUpload/:id', async(req, res) => {
    try {
        UserFiles.belongsTo(Client, {foreignKey: 'client'})
        let id = req.params.id;
        const location = await UserFiles.findOne({where: {id: id}, attributes: ['file_location']});
        const name = await UserFiles.findOne({where: {id:id}, attributes:['file_name']});
        const ResourceCategory = await UserFiles.findOne({where: {id:id}, attributes:['category']});
        const getClient = await UserFiles.findOne({where: {id:id}, include: Client, attributes:['client']})
