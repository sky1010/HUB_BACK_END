...
const Options = require('../sqlmodels/optionsModel');
const Links = require('../sqlmodels/linkModels');
...
router.get('/getUserOpts/:id', async (req, res) => {
    try {
        ...
        const user = await Options.findAll({ where: 
                                            {user: id},
                                            include: User
                                        })  
        res.json(user);
...
router.get('/getUserOpts/:id/:category', async (req, res) => {
    try {
        ...
        Options.belongsTo(User, {foreignKey: 'user'});
        const user = await Options.findOne({ where: 
                                            {user: id, category: cat},
                                            include: User
                                        })  
        res.json(user);
    } catch (err) {
...
router.get('/userLinks/:id/:cat', async (req, res) => {
    try {
        ...
        const links = await Links.findOne({where: {user: id, category: cat}, include: User})
        res.json(links)
    } catch (error) {
        res.json({Message: error.message})
    }
...
router.get('/usersLinks/:id/:cat', async (req, res) => {
    try {
        ...
        let cat = req.params.cat;
        Links.belongsTo(User, {foreignKey: 'user'});
        const links = await Links.findOne({where: {user: id, category: cat}, include: User, attributes: ['url', 'category']})
        res.json(links)
    } catch (error) {
        res.json({Message: error.message})
    }
