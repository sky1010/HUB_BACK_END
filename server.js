...
const UserFiles = require('./sqlmodels/userFileModel');
...
app.use('/en', express.static(__dirname + '/translations/en/translations.json'));
app.use('/fr', express.static(__dirname + '/translations/fr/translations.json'));
app.use(cors());
...
app.get('/ENGjson', async (req,res) => {
    try {
        const data = fs.readFileSync(__dirname+'/translations/en/translations.json')
   ...
   try {
    if(fs.existsSync(`${pathToFile}`)){
        ...
        fs.readFile(`${pathToFile}`, function(err, data) {
            ...
            delete json;
            fs.writeFileSync(`${pathToFile}`, JSON.stringify(newTranslations),"utf-8");
        })
    }
   }
   catch (err) {
...
app.post('/userUpload', async (req, res) => {
    try {
        if(!req.files) {
            ...
            if(fs.existsSync(`${mappath}`+'filemap_'+`${req.body.category}`+'_'+`${req.body.clientName}`+'.json')){
                let newData = {
                    "id": fileName,
                    "name": fileName,
                    "location": location,
                ...
                fs.readFile(`${mappath}`+'filemap_'+`${req.body.category}`+'_'+`${req.body.clientName}`+'.json', function(err, data) {
                    var json = [];
                    json = JSON.parse(data);
                    ...
                    fs.writeFileSync(`${mappath}`+'filemap_'+`${req.body.category}`+'_'+`${req.body.clientName}`+'.json', JSON.stringify(json))
                })
                const newFile = {
                    file_name: fileName,
                ...
                UserFiles.create(newFile)
                    .then(res.status(200).send({msg: "Success!", status: true,
                    data: file, code:200}))
                    .catch(err => {
                ...
                fs.writeFileSync(`${mappath}`+'filemap_'+`${req.body.category}`+'_'+`${req.body.clientName}`+'.json', fileMapJSON, 'utf8')
...
app.post('/filemap', async (req,res) => {
    try {
        ...
        if(fs.existsSync(`${mapPath}`+'filemap_'+`${category}`+'_'+`${client}`+'.json')) {
            fs.readFile(`${mapPath}`+'filemap_'+`${category}`+'_'+`${client}`+'.json', function(err, data) {
                var json = [];
                json = JSON.parse(data);
                delete json.fileMap;
                json["fileMap"] = fileMap
                console.log(json)
                fs.writeFileSync(`${mapPath}`+'filemap_'+`${category}`+'_'+`${client}`+'.json', JSON.stringify(json))
            })
        } else {
            let fileData = {
                "rootFolderId": "1",
            ...
            fs.writeFileSync(`${mapPath}`+'filemap_'+`${category}`+'_'+`${client}`+'.json', fileMapJSON, "utf-8");
        }
    }
    catch (err) {
